#!/usr/bin/env python3
"""Fetch latest upstream docs for client-config-opencode skill self-update.
...
def fetch_url(url: str) -> str:
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "opencode-skill-updater/1.0"},
    )
...
def main():
    ...
    urls_to_fetch: dict[str, list[str]] = {}  # url -> list of reference files that use it

    # Index URL
    index_url = sources.get("index")
    if index_url:
        urls_to_fetch.setdefault(index_url, []).append("index")

    # Schema URLs
    for key, url in sources.get("schema_urls", {}).items():
        urls_to_fetch.setdefault(url, []).append(f"schema:{key}")
