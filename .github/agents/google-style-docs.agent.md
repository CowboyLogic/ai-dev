...
When making structural changes to the 'docs/' folder (such as adding, removing, or reorganizing files), use the mkdocs-site-management skill to ensure proper MkDocs configuration and build validation.

Key steps from the skill:
1. Analyze the change and update the nav in mkdocs.yml if needed.
2. Run mkdocs build --clean --strict to validate.
3. Resolve any errors or warnings.
